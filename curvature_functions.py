...
#from scipy.special import rel_entrf
...
def compute_ricci_curvature(adj_mat: np.ndarray, threshold: float = 0.5, alpha: float = 0.5):
    ...
    g_generated = nx.from_numpy_array(adjacency_matrix) ## in this setting we don't have weight attribute for edges, we need to manually attribute the values to edges
...
def graph_vis(dist_mat: np.ndarray, threshold: float) -> None:
    ...
    if go is None:
        raise ModuleNotFoundError(
            "plotly is required for graph_vis(). Install it with `pip install plotly` "
            "or avoid calling graph_vis()."
        )
...
def visualise_box_plot(data1, data2, data3) -> None:
    """
    Visualise the datasets with box plots.

    This is used for comparing the curvature values.
    Plot 3 box plot in a same figure.
    """
    fig = go.Figure()
...
def ricci_flow(graph: nx.Graph, iteration: int = 30, alpha: float = 0.5, exp_power: int = 0, verbose="INFO",):
    """Compute Ricci flow for given graph."""
    orf = OllivierRicci(graph, weight="weight",alpha=alpha, method="OTD", chunksize=1, base=1, exp_power=0, verbose="INFO")
