...
def NeuRepsFig():
    ...
    normalized_colors = np.array(
        [[((w - min_weight) / (max_weight - min_weight)) for w in weight] for weight in weights]
    )
    # Scale to 1-6
    ...
    for j in range(4):
        ...
        nx.draw(
            ...
            ax=gaxs[j],
            node_size=20,
            width=normalized_widths[j],
            edge_color=normalized_colors[j],
            edge_cmap=cm.magma,
        )
        # Label
        ...
        gaxs[j].text(
            ...
            1.0,
            glabs[j],
            transform=(gaxs[j].transAxes + ScaledTranslation(-20 / 72, +7 / 72, fig.dpi_scale_trans)),
            fontsize=25,
            fontweight="bold",
            va="bottom",
        )
    ...
    ax1.text(
        ...
        1.0,
        "E",
        transform=(ax1.transAxes + ScaledTranslation(-20 / 72, +7 / 72, fig.dpi_scale_trans)),
        fontsize=25,
        fontweight="bold",
        va="bottom",
    )
    ...
    ax2.text(
        ...
        1.0,
        "F",
        transform=(ax2.transAxes + ScaledTranslation(-20 / 72, +7 / 72, fig.dpi_scale_trans)),
        fontsize=25,
        fontweight="bold",
        va="bottom",
    )
