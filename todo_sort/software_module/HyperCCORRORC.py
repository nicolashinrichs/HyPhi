        ...
        for trial_type in config["trial_types"]:
            ...
            for alph in config["orc_alphas"]:
                ...
                for powr in config["orc_powers"]:
                    ...
                    # Allocate memory
                    ORCvals = np.zeros(
                        (config["num_trials"], config["num_windows"], config["num_channels"], config["num_channels"])
                    )

                    Hvals = np.zeros((config["num_trials"], config["num_windows"]))

                    Qvals = np.zeros((config["num_trials"], config["num_windows"], len(config["quantiles"])))
