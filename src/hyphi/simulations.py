...
# ==========================
"""
Placeholder scripts for missing manuscript simulations.
...
def load_connectome(pickle_path):
    ...
    roi_names = [name.decode("utf-8") if isinstance(name, bytes) else name
                 for name in roi_names]

    return W, tract, roi_names


def create_virtual_partner_connectome(W, D, roi_names, C_inter=0.0):
...
def setup_delayed_kuramoto(W, tract, omega, c_intra=2.0, velocity=1.65,
                           noise_strength=0.1, seed=42):
    ...
    -------
    object
        Configured DDE solver (or placeholder).
    """
...
def run_delayed_kuramoto(solver, dt, t_max, n_osc, t_skip=2.0):
    """Integrate the delayed Kuramoto model and return phase trajectories.

    Parameters
    ----------
    solver : object
        Configured DDE solver from setup_delayed_kuramoto.
    dt : float
    ...
    """
    # TODO: Insert original simulation loop here.
    # For each time step:
    #   phases = solver.integrate(time) % (2 * np.pi)
    #   phases += noise_term
    #   compute order parameter r = |mean(exp(j * phases))|
...
def gen_weighted_sw(n, k, p, epsilon, seed=42):
...
def run_ws_sweep(n=1000, k=50, epsilon=1.0, t_rez=100,
                 min_pow=-4, max_pow=0, n_reps=200, seed_base=0):
    ...
    """
    from .curvatures import compute_frc_vec
    from .entropies import vec_entropy, vec_quantiles
    ...
    for rep in range(n_reps):
        ...
        Gt = [gen_weighted_sw(n, k, pt[t], epsilon, seed=seed_base + rep)
              for t in range(t_rez)]
