"""Compute per-window Forman-Ricci curvatures on CCORR inter-brain graphs and write per-trial entropies."""
...
if __name__ == "__main__":
    ...
    # Curvature entropies for each window in each frequency band for each dyad and trial type will be computed and
    # stored in 8 [frequency bands] x 30 [trials] x 4 [consecutive windows]
    # Curvature quantiles for each window in each frequency band for each dyad and trial type will be computed and
    # stored in 8 [frequency bands] x 30 [trials] x 4 [consecutive windows] x 5 [quantiles]
    ...
    for dyad in tqdm(config["dyads"], desc="Dyads"):
        ...
        for trial_type in tqdm(config["trial_types"], desc="Trial Types"):
            ...
            Hvals = np.zeros((config["num_freqs"], config["num_trials"], config["num_windows"]))

            Qvals = np.zeros(
                (config["num_freqs"], config["num_trials"], config["num_windows"], len(config["quantiles"]))
            )
            ...
            for trial in tqdm(range(config["num_trials"]), desc="Trials"):
                ...
                for freq in tqdm(range(config["num_freqs"]), desc="Frequency Bands"):
                    ...
                    for window in range(4):
                        FRCvals[freq, trial, window, :, :] = nx.attr_matrix(FRCt[window], edge_attr="formanCurvature")[
                            0
                        ]
