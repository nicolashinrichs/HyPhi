...
def _require_plotly():
    if go is None:
        raise ModuleNotFoundError(
            "plotly is required for this visualization function. "
...
def visualize_graph_with_curvature(
    ...
    nx.draw_networkx_nodes(
        graph, pos, node_color="gray", node_size=5, alpha=0.3
    )
...
def visualize_graph_partitions_colors(graph: nx.Graph, partitions, name: str, save=True):
    ...
    for community in unique_communities:
        ...
        nx.draw_networkx_nodes(graph, pos, nodelist=nodes_in_community,
                               node_color=[color] * len(nodes_in_community),
                               node_size=20, alpha=0.5, label=f'Community {community}')
    ...
    nx.draw_networkx_edges(graph, pos, width=2, edge_color=edge_colors, edge_cmap=edge_palette, edge_vmin=-1, edge_vmax=1)
...
def visualize_graph_on_dataset_with_colors(G, pos, partitions):
    ...
    legend_handles = [plt.Line2D([0], [0], marker='o', color='w', label=f'Community {community}',
                                 markerfacecolor=community_to_color[community], markersize=10)
                      for community in unique_communities]
