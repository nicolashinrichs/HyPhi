...
notebooks in ``experiments/notebooks/`` use these functions to render:
...
_EXPECTED_NDIM = 2  # pairwise GDD matrix must be 2-D
...
def plot_weight_distributions_by_matrix(
    ...
    for ax, key in zip(axes_flat, keys, strict=False):  # lengths differ when n < nrows*ncols
...
def plot_gdd_heatmap(
    ...
    *,
    ...
    if arr.ndim != _EXPECTED_NDIM or arr.shape[0] != arr.shape[1]:
