...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-dir", type=Path, default=None, help="Directory containing *_connectome_kuramoto.pkl files."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--target-windows", type=int, default=None, help="Target number of time windows (default from config)."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional PLV threshold (e.g., 0.2) to sparsify merged graphs.",
    )
...
def main() -> None:
    ...
    if args.graph_construction == "merge_graphs":
        ...
        for rid in runs:
            ...
            if phase_path.exists():
                ...
                print(
                    f"[load] run={rid} phase_windows={len(phase_windows_by_run[rid])} first_shape={first_shape}",
                    flush=True,
                )
            else:
    ...
    est_nodes = len(runs) * nodes_per_run  # node-count estimate for the log line below
    print(
    ...
    for widx in range(n_windows):
        ...
        if args.graph_construction == "merge_graphs":
            ...
            if agg_graph.number_of_nodes() != expected_nodes:
                raise RuntimeError(f"Merged graph node mismatch: {agg_graph.number_of_nodes()} != {expected_nodes}")
        ...
        # Per-window cluster visualization is deferred behind a future --enable-visualization flag (see #110).
