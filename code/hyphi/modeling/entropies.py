...
# Sentinel returned for a degenerate curvature distribution (empty, a single value, or all
# identical). Such a distribution has no meaningful differential entropy, and an unguarded
# estimator either raises or returns NaN/inf (the KDE bandwidth selection divides by zero).
# Returning 0.0 reads the case as "no disorder" and keeps the entropy-over-time trace finite.
# This matches the prior guarded behaviour in analyses.py. Whether the sentinel should instead
# be NaN or an explicit error is a deliberate scientific choice that is still open.
_DEGENERATE_ENTROPY = 0.0

# Minimum number of distinct curvature values required to estimate entropy. Fewer than this
# (an empty array, a single value, or an all-constant distribution) is degenerate.
