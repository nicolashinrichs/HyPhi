...
# Minimum number of graphs required to compute any pairwise or successive distance.
_MIN_GRAPHS = 2


def compute_global_curvature_stats(curvature_dict):
    """Pool curvature arrays from all graphs and compute summary statistics.

    Parameters
    ----------
    curvature_dict : dict
        Dictionary mapping graph identifiers to curvature arrays (any shape).
    ...
    dict
        ...
        - ``all_curvatures``: flattened array of every curvature value.
        - ``mean``: global mean (float).
        - ``std``: global standard deviation, ddof=0 (float).
        - ``median``: global median (float).
        - ``mad``: median absolute deviation from the median (float).

    Raises
    ------
    ValueError
        If ``curvature_dict`` yields no curvature values.
    """
...
def compute_frc_bundle_from_adjacency(adjacency, method="1d"):
    """Compute a Forman-Ricci curvature bundle from a single adjacency matrix.
    ...
    method : str, optional
        Method passed to :func:`compute_frc`. Default is ``"1d"``.
    ...
    dict
        ...
        - ``adjacency``: numpy array copy of the input matrix.
        - ``graph``: NetworkX graph built from the adjacency.
        - ``graph_frc``: NetworkX graph with Forman-Ricci curvature attributes.
        - ``edge_curvatures``: dict mapping ``(u, v)`` pairs to curvature values.
        - ``curvatures``: 1-D float array of all edge curvatures.
    """
