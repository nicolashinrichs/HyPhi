"""End-to-end smoke tests: density estimation, the FRC-entropy pipeline, config init."""
...
def test_config_init_does_not_change_cwd(tmp_project, monkeypatch):
    """config.init() must not move the caller's working directory."""
    # Regression: config.init() used to chdir to the project root as a side
    # effect, silently breaking every relative path in the calling script.
    subdir = tmp_project / "scripts"
    subdir.mkdir()
    monkeypatch.chdir(subdir)
    config.init()
    assert Path.cwd() == subdir
    # Relative config paths are resolved to absolute against the project root,
...
def test_bootstrap_chdirs_to_project_root(tmp_project, monkeypatch):
    """bootstrap() is the explicit opt-in: init plus chdir to the project root."""
    subdir = tmp_project / "scripts"
    subdir.mkdir()
    monkeypatch.chdir(subdir)
    cfg = bootstrap()
