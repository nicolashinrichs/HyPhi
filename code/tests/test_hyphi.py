...
def test_fit_kde_isj_recovers_normal_density(gaussian_sample):
    # Sheather-Jones (ISJ) bandwidth selection should recover the standard
    # normal density well, and better than a deliberately oversmoothed bw=1
    # (the optimal bandwidth for n=1000 standard-normal draws is ~0.27, not
    # the data's standard deviation). Grids differ per bandwidth, so each
    # estimate is compared to the true pdf on its own grid.
    x_isj, y_isj = fit_kde(gaussian_sample, bw="ISJ", method="tree")()
    x_wide, y_wide = fit_kde(gaussian_sample, bw=1, method="tree")()

    assert np.all(np.isfinite(y_isj)) and np.all(y_isj >= 0)
    assert np.trapz(y_isj, x_isj) == pytest.approx(1.0, abs=0.01)
    assert np.trapz(y_wide, x_wide) == pytest.approx(1.0, abs=0.01)
...
def test_sw_frc_entropy_pipeline():
    # The Nature methods small-world pipeline end to end (generate graphs,
    # Forman-Ricci curvature, entropies) at toy scale; gen_nature_sw() runs
    # the same chain at full size (1000 nodes, 100 graphs) in minutes.
    # Rewiring probabilities in [0.1, 1]: near-lattice graphs (p ~ 1e-4) have
    # almost constant curvature, where the ISJ bandwidth solver is known to
    # fail (the degenerate-entropy safeguard is tracked in issue #28).
